#property copyright   "GPT-5 Assistant"
#property link        "https://"
#property version     "1.0"
#property description "ETHUSD M1 VWAP + Heikin Ashi + Doji + ATR SL/TP + Sessions + Risk"
#property strict
...
// News filter (requires broker calendar support)
input bool     InpEnableNewsFilter        = false;             // Use economic calendar filter
input int      InpNewsCloseMinsBefore     = 10;                // Close trades X minutes before high-impact news

// Misc
input uint     InpMagic                   = 2025082501;        // Magic number
...
bool HasConsecutiveWicklessRun(bool bearish, int lookback, const double &ha_open[], const double &ha_close[], const double &ha_high[], const double &ha_low[])
{
	...
	for(int i=1; i<=lookback; ++i)
	{
		...
		else
		{
			...
			if(opposite) found_run = 0; else found_run = 0; // reset either way when condition breaks
		}
	}
	return(false);
}
...
void PlaceOrder(bool is_buy, double sl_price, double tp_price, const string comment)
{
	...
	bool ok = false;
	if(is_buy)
		ok = Trade.Buy(lot, g_symbol, 0.0, sl_price, tp_price, comment);
	else
		ok = Trade.Sell(lot, g_symbol, 0.0, sl_price, tp_price, comment);
	if(!ok)
	{
...
void ManageOpenPosition()
{
	...
	// Trailing after partial is enabled: continue to trail
	if(InpEnableTrailingAfterPTP)
	{
...
int OnInit()
{
	if(!GetSymbolProps(InpSymbol))
	{
		Print("Failed to get symbol props: ", InpSymbol, " error=", _LastError);
		return(INIT_FAILED);
	}
	Trade.SetExpertMagicNumber(InpMagic);
